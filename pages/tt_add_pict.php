...
<?php
     ...
     function addproduct(){
                ...
                ///////cette partie concerne l'ajjout d'un repas'
                                ...
                                $nomprod =  htmlentities($_POST['le_nomprod']);
                                $typeprod = htmlentities($_POST['le_typeprod']);
                                $imgname = basename($_FILES["image"]["name"]);
                                $img_forma=basename($_FILES["image"]["type"]);
                                $descriptionprod =htmlentities($_POST['le_description']); // 0: pour l'utilisateur par défaut 1:...
                                 
                                
                                if ($stmt = $mysqli->prepare("INSERT INTO product(nom_p, categorie, commentaire,img) VALUES (?, ?, ?,?)")) {
                                    $stmt->bind_param("ssss", $nomprod, $typeprod, $descriptionprod,$imgname);
...
<?php
   function productimg(){
                    ...
                    $target_dir = "images/"; // répertoire de destination pour enregistrer l'image
...
<?php
        function add_b_file(){
                    ...
                    $stmt = $mysqli->prepare("INSERT INTO product (image_file) VALUES (?)");
